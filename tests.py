...
def test_sigmoid_0():
    try:
        ...
        grad_error = np.linalg.norm(torch_X.grad.detach().numpy() - sigmoid_model.network_operations[1].last_forward_result['df']['dfdx'])
...
def test_sequential_0():
    try:
        ...
        grad_error = np.linalg.norm(model.network_operations[2].params['A'].grad - torch_model.layer2.weight.grad.detach().numpy())
        grad_error += np.linalg.norm(model.network_operations[0].params['A'].grad - torch_model.layer1.weight.grad.detach().numpy())
        grad_error += np.linalg.norm(model.network_operations[0].params['b'].grad - torch_model.layer1.bias.grad.detach().numpy())
        grad_error += np.linalg.norm(model.network_operations[2].params['b'].grad - torch_model.layer2.bias.grad.detach().numpy())
        grad_error /= 4
