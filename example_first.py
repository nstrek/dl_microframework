...
def convert_prob_into_class(Y_pred):
    ...
    res[np.argmax(Y_pred)] = 1
    return res


def get_accuracy_value(Y_pred, Y):
    Y_pred_ = convert_prob_into_class(Y_pred)
...
def full_backward_propagation(Y_pred, Y, memory, params_values, nn_architecture):
    grads_values = {}
    m = Y.shape[1]
...
def train(X, Y, nn_architecture, epochs, learning_rate):
    ...
    for i in range(epochs):
        ...
        cost = get_cost_value(Y_pred, Y)
        cost_history.append(cost)
        accuracy = get_accuracy_value(Y_pred, Y)
        accuracy_history.append(accuracy)

        # print(Y_pred.shape, Y.shape)

        grads_values = full_backward_propagation(Y_pred, Y, cashe, params_values, nn_architecture)
...
Y = np.sum(5 * X + 1, axis=0, keepdims=True)
Y = np.array(Y > np.average(Y), dtype=np.float32)
...
print(train(X, Y, nn_architecture, 100, 1e-1 * 10))
